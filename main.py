        ...
        if args_enc[0] == "y":  # если функция первым аргументом принимает строку "у"(eng)
            ...
            for obj in x_matrix:
                ...
                for i in range(len(args_enc)):
                    sum_i_temp *= obj[
                        args_enc[i] - 1]  # перемножаем все Х из кортежа, для квадрата дважды добавляем в кортеж Х
                summa += sum_i_temp
...
def y_func(x1, x2, x3):
    return 6.7 + 6.5 * x1 + 1.3 * x2 + 6.5 * x3 + 2.4 * x1 * x1 + 0.8 * x2 * x2 + 9.6 * x3 * x3 + 2.6 * x1 * x2 + 0.9 * x1 * x3 + 5.8 * x2 * x3 + 2.8 * x1 * x2 * x3
...
x1min, x1max = -15, 30  # начальные условия
x2min, x2max = 25, 65
x3min, x3max = -15, 5
m = 3
N = 15
...
print("Yi:", y_list)
for i in range(100): # Зацикливаем
    for num_of_coef in num_of_coef_list:  # ймовірний перехід до рівняння регресії з урахуванням ефекту взаємодії та квадратичних членів
        ...
        for i in range(N):  # подсчёт и вывод у с шляпокой с учётом вида уравнения регрессии
            if num_of_coef == num_of_coef_list[0]:
                y_hat.append(
                    b_coefs[0] + b_coefs[1] * x_matrix[i][0] + b_coefs[2] * x_matrix[i][1] + b_coefs[3] * x_matrix[i][2])
            elif num_of_coef == num_of_coef_list[1]:
                y_hat.append(
                    b_coefs[0] + b_coefs[1] * x_matrix[i][0] + b_coefs[2] * x_matrix[i][1] + b_coefs[3] * x_matrix[i][2] +
                    b_coefs[4] * x_matrix[i][0] * x_matrix[i][1] + b_coefs[5] * x_matrix[i][0] * x_matrix[i][2] +
                    b_coefs[6] * x_matrix[i][1] * x_matrix[i][2] + b_coefs[7] * x_matrix[i][0] * x_matrix[i][1] *
                    x_matrix[i][2])
            else:
                y_hat.append(
                    b_coefs[0] + b_coefs[1] * x_matrix[i][0] + b_coefs[2] * x_matrix[i][1] + b_coefs[3] * x_matrix[i][2] +
        ...
        if Fp > Ft:  # если Fp > Ft, то выполняется этот блок
            ...
            if num_of_coef == num_of_coef_list[0]:
                counter1 += 1 #Підраховуємо кількість з ефектом взаємодії
                # print("Переходимо до рівняння з урахуванням ефекту взаємодії.")
            elif num_of_coef == num_of_coef_list[1]:
                counter2 += 1 #Підраховуємо кількість з урахуванням квадратичних членів.
